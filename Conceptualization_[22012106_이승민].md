...
* PROPAGATOR — 확률적 CA와 앙상블 시뮬레이션을 결합하여 확률 히트맵을 제공하나, 산불에 특화되어 건물 내부 화재에는 적용할 수 없다.
...
건물 구조 및 재질 정보를 기반으로 확률적 셀룰러 오토마타 앙상블 시뮬레이션을 수행하고, 그 결과를 확률 히트맵으로 시각화하여 소방 관제관과 현장 요원들의 작전 판단을 지원하는 시뮬레이터를 개발한다. 본 시스템은 다음의 핵심 기능을 제공한다.

건물 평면도 및 구조·재질 데이터를 기반으로 한 화재 확산 시뮬레이션
확률적 CA의 앙상블 실행을 통한 셀별 화재 도달 확률 산출
층별 확률 히트맵 시각화 및 건물 단면도 위험도 요약
스프링클러·방화셔터 등 소방 설비 조건에 따른 What-if 시나리오 비교
...
| --- | --- |
| **Approach** | 관제관 또는 현장요원이 시뮬레이션 시점을 조절하여 특정 시간대의 확률 분포를 확인할 수 있다. 슬라이더 또는 입력 필드를 통해 시뮬레이션의 특정 tick을 지정하면, 해당 시점의 확률 히트맵이 표시된다 |
...
본 시스템은 사전 등록된 건물 데이터에 의존한다. 미등록 건물에 대해서는 시뮬레이션을 수행할 수 없다는 기술적 한계가 존재한다. 다만 현재 BIM 의무화가 단계적으로 시행되고 있으며, 이와 연동하여 점진적으로 해소될 것으로 기대된다.
