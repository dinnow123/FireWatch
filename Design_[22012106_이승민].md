...
- **EMPTY → EMPTY** (자기 전이): 셀은 EMPTY 상태에서 이웃 BURNING 셀로부터 heat를 누적한다(addHeat). 누적된 heat가 재질의 발화 임계값(threshold)에 도달하지 않는 한 EMPTY 상태를 유지한다.
