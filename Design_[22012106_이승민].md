...
이 입장은 임의의 선택이 아니라 화재 현상의 물리적 본성에서 도출된 것이다. 화재 확산은 열역학 법칙, 푸리에 열전도 법칙, 연소 화학 반응, 유체 운동 방정식 등 결정론적 물리 법칙의 상호작용으로 지배된다. 원리적으로는 초기 조건과 미시 상태가 빠짐없이 주어진다면 단일한 확산 경로가 도출되어야 한다. 본 시스템이 확률을 도입하는 이유는 이 결정성을 부정하기 때문이 아니라, 그 미시 상태를 실제로 관측할 수 없기 때문이다.
