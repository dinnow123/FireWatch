# Design Document — 1. Introduction (초안)

> Analysis(2번 과제)의 Executive Summary를 인용하며, 본 시스템의 한계와 나아갈 방향을 함께 제시한다.
...
### 1.2. 설계 철학 — 인식론적 결정론 (불확실성의 정량화)

본 시스템의 핵심 설계 철학은 인식론적 결정론(Epistemic Determinism)이다. 시뮬레이션 엔진 자체는 결정론적 시스템이지만(동일 seed와 동일 입력에 대해 항상 동일한 출력), 시스템이 산출하는 "발화 확률"은 물리적 무작위성이 아니라 관측 불가능한 변수들(미세 공기 흐름, 가구 배치, 재질의 미세 상태 등)에 대한 정보 부족, 즉 무지(無知)를 정량화한 값이다. 이는 기상예보의 "강수확률 80%"와 동일한 원리로, 현상 자체는 결정론적이되 관측자의 정보가 불완전하므로 그 정보 부족을 확률 분포로 표현하는 입장이다.

이 철학은 시스템 구조에 직접 반영된다. EnsembleRunner가 서로 다른 seed로 N회 시뮬레이션을 수행하여 확률 분포를 추정하고, EnsembleResult가 Wilson Score 신뢰구간을 함께 산출한다. 나아가 HeatmapView는 신뢰구간의 폭을 시각화하여, 결정론적으로 확실한 영역(발화점 인근, 안전 구역)과 진정으로 불확실한 경계 영역을 구분해 보여준다. 이로써 사용자는 의사결정에 실질적으로 중요한 경계 영역에 주의를 집중할 수 있다.
...
SimulationAdapter가 UI와 Engine 사이의 유일한 통로 역할을 하여, 시뮬레이션 로직이 특정 UI 기술에 묶이지 않는다. 아래에서는 각 계층의 세부 클래스 구조를 차례로 설명한다.


![전체classdiagram](images/allclassdiagram.drawio.png)
...
격자 셀 클래스(1m × 1m). 실제 구현에서는 개별 객체가 아니라 Floor의 numpy 배열의 한 원소로 표현되어 벡터화 연산의 대상이 된다. `state`는 EMPTY·BURNING·BURNED·BLOCKED 네 상태의 열거형 의미이며, 구현에서는 성능을 위해 정수(0~3)로 표현한다(Material과 동일).
...
**Attributes**
- `CONCRETE, GYPSUM, WOOD, TEXTILE, PLASTIC, PAPER`
...
**Methods**
- `+ isSprinklerActive(): bool`
- `+ isShutterActive(): bool`
...
CAEngine을 동일 조건으로 N회 반복 호출하여 도달 확률 분포를 산출한다. 서로 다른 seed로 무지를 샘플링하는 인식론적 결정론을 구현한다.

**Attributes**
- `- building` — 대상 건물
- `- scenario` — 설정
- `- nRuns: int` — 앙상블 회차 수
- `- fireCounts` — 층·셀별 누적 점화 카운트
...
**Attributes**
- `- probabilityMaps` — 층·셀별 도달 확률 (0~1)
- `- nRuns: int` — 앙상블 회차 수
...
**Attributes**
- `- sprinklerChk` — 스프링클러 체크박스
- `- shutterChk` — 방화셔터 체크박스
...
결과 시각화의 핵심 화면. 셀별 도달 확률을 색상 그라데이션으로, 신뢰구간 폭을 해칭 패턴으로 시각화하여 결정론적으로 확실한 영역과 진정으로 불확실한 경계 영역을 구분해 보여준다.
...
**Attributes**
- `- leftCanvas` — 좌측 렌더링 영역
- `- rightCanvas` — 우측 렌더링 영역
...
**Attributes**
- `- buildingsDB` — 건물 데이터 저장소
- `- scenariosDB` — 시나리오 저장소
...
- **인식론적 결정론**: 엔진은 결정론적이나 출력 "확률"은 무지(관측 불가능한 변수에 대한 정보 부족)의 정량화다. EnsembleRunner가 서로 다른 seed로 N회 반복하는 구조에 반영된다.
