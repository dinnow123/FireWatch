...
UI 계층과 Engine 계층 사이를 중개하는 파사드(Facade) 클래스다. View들이 EnsembleRunner·CAEngine 같은 Engine 내부 클래스를 직접 호출하지 않고 오직 본 클래스의 공개 메서드만 사용하도록 하여, UI와 Engine 사이의 결합도를 낮추고 단방향 의존(UI → Engine)을 강제하는 역할을 한다. 또한 UI가 다루는 입력 형식(주소 문자열, 화면 입력값 등)과 Engine이 요구하는 도메인 객체(Building, Scenario) 사이의 데이터 변환과 함수 시그니처 호환을 담당한다. 이로써 Engine의 내부 구조가 바뀌더라도 그 영향이 본 클래스 내부에 격리되어 UI 코드는 변경할 필요가 없으며, 반대로 UI 기술(PyQt6)이 교체되더라도 Engine은 영향을 받지 않는다. 상태를 보유하지 않는 무상태(stateless) 파사드로 설계되었다.
