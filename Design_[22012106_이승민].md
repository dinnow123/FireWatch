...
본 장은 시스템의 동적 상태 변화를 상태 기계 다이어그램으로 정의한다. FireWatch에는 서로 다른 수준의 상태 기계가 두 개 존재한다. 하나는 시뮬레이션 세션 전체의 상태 전이이고, 다른 하나는 시뮬레이션의 최소 단위인 셀(Cell)의 상태 전이다. 두 상태 기계는 독립적이지 않고 계층적으로 중첩되는 관계에 있다. 즉 세션이 시뮬레이션을 수행하는 동안 그 내부에서 모든 셀이 각자의 상태 기계를 병행하여 갱신한다. 따라서 본 장은 먼저 두 수준을 함께 담은 통합 상태 다이어그램으로 전체 구조를 개관한 뒤, 셀과 세션의 상태 전이를 각각 분리하여 상세히 설명한다.

---

### 4.1. 통합 상태 다이어그램

![통합 상태 다이어그램](stateDiagram/4-3.png)

*[그림 4-1] FireWatch 통합 상태 다이어그램*

위 그림은 시스템의 두 수준 상태 기계를 하나로 종합한 것이다. 바깥 계층은 시뮬레이션 세션의 상태(IDLE, RUNNING, COMPLETED)이며, 세션이 RUNNING 상태에 진입하면 그 내부에서 매 tick마다 전체 셀의 상태 기계가 갱신된다. 즉 셀 상태 기계(EMPTY, BURNING, BURNED, BLOCKED)는 세션의 RUNNING 상태에 중첩된 하위 상태 기계로 동작한다. 이 중첩 구조는 "세션이라는 거시적 흐름 안에서 무수한 셀의 미시적 전이가 동시에 일어난다"는 본 시스템의 시뮬레이션 구조를 직접적으로 드러낸다.

이 종합도는 전체 관계를 한눈에 조망하기 위한 것이며, 각 수준의 전이 조건은 아래에서 셀(4.2)과 세션(4.3)으로 나누어 차례로 상세히 설명한다.

---

### 4.2. 셀(Cell) 상태 전이

![셀 상태 전이](stateDiagram/4-2.png)

*[그림 4-2] 셀(Cell) 상태 전이 다이어그램*

위 그림은 한 셀이 시뮬레이션 동안 거치는 상태 전이를 표현한 것이다. 셀은 네 가지 상태(EMPTY, BURNING, BURNED, BLOCKED)를 가진다.
...
- **EMPTY → BURNING**: 누적 heat가 임계값 이상이 되면, 발화 확률 p에 따라 확률적으로 BURNING 상태로 전이한다. 이 확률적 전이가 본 시스템의 핵심 메커니즘으로, 관측 불가능한 변수에 대한 정보 부족을 표현한다.
- **BURNING → BURNING** (자기 전이): BURNING 상태의 셀은 매 tick마다 연소 시간(burnTimer)을 증가시키며 이웃으로 heat를 방출한다.
- **BURNING → BURNED**: 연소 시간이 재질의 연소 지속 시간(burnDuration)에 도달하면 BURNED 상태로 전이하여 더 이상 heat를 방출하지 않는다.
- **EMPTY → BLOCKED**: 방화셔터가 설치된 셀에서 heat가 작동 임계값(shutterTriggerHeat)에 도달하면, 해당 셀은 BLOCKED 상태로 전이하여 화재 전파를 차단한다. 벽으로 지정된 셀은 시뮬레이션 시작 시점부터 BLOCKED 상태다.
...
![시뮬레이션 상태](stateDiagram/4-1.png)

*[그림 4-3] 시뮬레이션 세션 상태 다이어그램*

위 그림은 시뮬레이션 세션 전체가 거치는 상태 전이를 표현한 것이다. 세션은 세 가지 주요 상태(IDLE, RUNNING, COMPLETED)를 가진다.

- **IDLE**: 시스템 초기 상태로, 신고 입력과 파라미터 설정을 기다린다.
- **IDLE → RUNNING**: 관제관이 입력을 완료하고 실행을 요청하면(requestRun), 앙상블 시뮬레이션이 시작되어 RUNNING 상태로 전이한다.
- **RUNNING → COMPLETED**: N회 반복이 모두 완료되면 결과가 산출되고 COMPLETED 상태로 전이하여 결과 조회가 가능해진다.
- **RUNNING → IDLE**: 시뮬레이션 도중 사용자가 취소하면(cancel) 진행 결과를 폐기하고 IDLE로 복귀한다.
- **COMPLETED → RUNNING**: 시나리오 비교 시 새 파라미터로 시뮬레이션을 재실행하면 다시 RUNNING 상태로 전이한다.
- **COMPLETED → 종료**: 관제관이 초기화하면(reset) 현재 세션 상태가 종료된다.
