...
| 2026.06.03 | 0.2 | 미구현 파라미터(환기구, 비상구) 등 삭제 | 이승민 |
...
이러한 한계를 보완하기 위해 본 프로젝트에서는 FireWatch 시스템을 제안한다. 선행 산불 시뮬레이터인 'PROPAGATOR'가 채택한 확률적 CA + 앙상블 방법론을 건물 화재 도메인에 적용하여, 동일한 조건에서 다수의 시뮬레이션을 수행하고 그 결과를 종합함으로써 각 구역별 화재 도달 확률을 산출한다. 이를 통해 단일 결과가 아닌 확률 분포 형태의 예측을 제공하며, 사용자는 위험도가 높은 구역을 직관적으로 식별할 수 있다.
...
Conceptualization 단계의 System context diagram과 Use case list를 참조하여 Use case diagram을 다음과 같이 작성하였다. 모델링 도구는 draw.io을 사용하였으며, 액터(Commander, Field Operator)와 11개의 Use Case 사이의 관계를 나타내었다.
...
|---|---|
| **GENERAL CHARACTERISTICS** | |
| Summary | 관제관이 시뮬레이션의 조건(스프링클러 작동 여부, 방화셔터 작동 여부)을 설정하는 기능 |
| Scope | FireWatch |
| Level | User Level |
| Author | |
| Last Update | |
...
| **EXTENSION SCENARIOS** | |
| Step | Branching Action |
| - | 본 Use Case는 별도의 예외 분기가 발생하지 않는다. |
| **RELATED INFORMATION** | |
...
|---|---|
| **GENERAL CHARACTERISTICS** | |
| Summary | 관제관이 다른 파라미터(설비 작동 여부 등)로 시뮬레이션을 재실행하고, 두 결과를 시각적으로 비교하는 기능 |
| Scope | FireWatch |
| Level | User Level |
| Author | |
| Last Update | |
...
| Failed Post Condition | 비교 화면이 표시되지 않아 시나리오 비교를 수행할 수 없다. |
| **MAIN SUCCESS SCENARIO** | |
| Step | Action |
...
1) User
사용자 클래스. Commander와 FieldOperator의 일반화이다. 기본 권한으로 시뮬레이션 결과의 조회 기능을 가진다.

2) Commander
관제관 클래스. User의 기능에 더해 신고 입력, 시뮬레이션 실행, 시나리오 비교, 초기화 등 모든 제어 권한을 가진다.

3) FieldOperator
현장요원 클래스. User의 기능 중 결과 조회와 자기 디바이스에서의 층 전환·시간 조작만 수행할 수 있다.

4) Building
건물 클래스. 평면도, 층 정보, 격자 셀 구조, 소방 설비 위치를 포함한다. Database로부터 로드되어 시뮬레이션의 입력이 된다. Floor와 FireSafetyEquipment를 가진다.

5) Floor
층 클래스. 건물의 한 층을 나타내며 격자 형태의 Cell들로 구성된다. 층 식별자(예: B1, 1F, 2F)를 가진다.

6) Cell
격자 셀 클래스. 평면도의 한 칸을 나타낸다. 위치 좌표(x, y), 재질, 그리고 시뮬레이션 결과로 산출되는 화재 도달 확률을 가진다.

7) Material
재질 클래스. 셀의 발화 임계값과 연소 속도 등 물성을 정의하는 enum 클래스다. (예: 콘크리트, 목재, 카펫, 가연성 가구)

8) FireSafetyEquipment
소방 설비 클래스. 스프링클러, 방화셔터 등의 설치 위치와 종류를 나타낸다. 건물에 종속된 정적 정보로 Database에 저장되며, 작동 여부는 SimParameters에서 별도로 결정된다.

9) Report
신고 클래스. 신고된 건물 식별자, 발화 좌표(x, y, 층), 발화 시각을 가진다. 신고 입력 시 생성되어 시뮬레이션의 입력이 된다.

10) SimParameters
시뮬레이션 파라미터 클래스. 시뮬레이션 환경 조건을 boolean 값으로 가진다(스프링클러, 방화셔터 각각의 작동 여부). 사용자가 직접 설정할 수 있으며, 미설정 시 모두 작동 상태(기본값)를 사용한다. 시나리오마다 다른 동적 정보로, 같은 건물에 대해 여러 시나리오를 비교하는 단위가 된다.

11) EnsembleResult
앙상블 결과 클래스. 20–50회 시뮬레이션 회차의 결과를 종합한 셀별 도달 확률 분포와 시점별 상태를 가진다. EnsembleRunner의 실행 결과로 산출된다.

12) Scenario
시나리오 클래스. 하나의 SimParameters와 그 결과인 EnsembleResult를 하나의 단위로 묶는다. 앙상블 실행 시마다 새로운 Scenario가 생성되며, 두 Scenario 간 셀별 확률 차이(Δ)를 산출하여 비교하는 단위가 된다.

13) CAEngine
셀룰러 오토마타 시뮬레이션 엔진 클래스. Cell의 이웃 상태, 재질, SimParameters를 기반으로 다음 시점의 화재 상태를 확률적으로 계산한다. 단일 회차의 시뮬레이션을 담당하며, 동일 입력으로도 매번 다른 결과를 산출한다.

14) EnsembleRunner
앙상블 실행기 클래스. CAEngine을 동일 조건으로 20–50회 반복 호출하고, 각 회차에서 셀별 점화 여부를 누적한다. 누적 결과를 회차 수로 나누어 셀별 도달 확률 분포(0–100%)를 산출하며, 이것이 EnsembleResult가 된다. 본 시스템의 핵심 시뮬레이션 로직을 수행한다.

15) DatabaseManager
데이터 관리 클래스. 시스템 내부 데이터 저장소와의 통신을 담당한다. Building 데이터의 조회, EnsembleResult의 저장 및 불러오기를 수행한다.
...
![Report Input Screen](images/report.png)
...
관제관이 시뮬레이션의 환경 조건을 설정하는 화면이다. 2개의 boolean 파라미터(스프링클러, 방화셔터)의 작동 여부를 입력한다.

![Parameters Screen](images/parameter.png)
...
![Heatmap Screen - Ignition Floor](images/heatmap.png)

![Heatmap Screen - Ignition Floor](images/heatmap2.png)

![Heatmap Screen - Ignition Floor](images/heatmap3.png)

시간 조절에 따라 셀들의 색이 변화한다. 이를 통해 화재의 진행 수준, 규모 등 예측할 수 있다
...
![Heatmap Screen - Other Floor](images/heatmap4.png)
...
![Section Screen](images/section.png)
...
![Comparison Screen](images/comparison.png)

#### 4.9.1. 사용 방법

1. 상단의 2개 체크박스로 비교 시나리오의 파라미터를 설정한다. 진입 시 기본 파라미터의 반대값이 자동 세팅된다.
...
![Field Operator Mode](images/field_mode.png)
...
관제관이 현재 진행 중인 시뮬레이션을 종료하고 모든 입력값과 결과를 초기 상태로 되돌리는 기능이다. 사이드바의 "초기화" 메뉴를 누르면 확인 다이얼로그가 표시되어 의사를 재확인한다.
...
| **Δ (Delta)** | 두 시나리오 간 셀별 도달 확률 차이를 나타내는 기호. 발산형 색상(빨강: 비교가 더 위험, 파랑: 비교가 더 안전)으로 시각화된다. |
...
3. Trucchia, A. et al., "PROPAGATOR: An Operational Cellular-Automata Based Wildfire Simulator", *Fire*, 3(3), 26, 2020 — https://doi.org/10.3390/fire3030026
