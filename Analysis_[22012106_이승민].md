...
| --- | --- | --- | --- |
| 2026.03.27 | 0.1 | Conceptualization | 이승민 |
| 2026.05.07 | 0.2 | Analysis Document | 이승민 |
...
![FireWatch Use Case Diagram](images/Use_case_diagram.png)

#### 2.1.1. Use case 관계 정리

| 관계 | 개수 | 내용 |
| --- | :---: | --- |
| «include» | 3 | UC1 → UC2, UC10 → UC3, UC10 → UC4 |
| «extend» | 2 | UC6 → UC5, UC7 → UC5 |
| Association | 15 | Commander 10개 + Field Operator 5개 |

#### 2.1.2. Actor별 접근 가능 Use Case 매트릭스

| Use Case | Commander | Field Operator |
| --- | :---: | :---: |
| UC1 Input Report | ✓ | |
| UC2 Load Building | (UC1 include) | |
| UC3 Set Parameters | ✓ | |
| UC4 Run Ensemble | ✓ | |
| UC5 Display Heatmap | ✓ | ✓ |
| UC6 Switch Floor | ✓ | ✓ |
| UC7 Control Time | ✓ | ✓ |
| UC8 View Building Section | ✓ | ✓ |
| UC9 View Result Summary | ✓ | ✓ |
| UC10 Compare Scenarios | ✓ | |
| UC11 Reset Simulation | ✓ | |
...
| --- | --- |
| **GENERAL CHARACTERISTICS** | |
| Summary | 시스템이 입력된 신고 정보와 파라미터를 바탕으로 동일 조건의 시뮬레이션을 다회 반복 실행하고, 그 결과를 종합하여 셀별 화재 도달 확률을 산출하는 기능 |
| Scope | FireWatch |
| Level | User Level |
| Status | Analysis |
| Primary Actor | Commander |
...
| Success Post Condition | 셀별 도달 확률 분포가 산출되어 시각화 단계에서 사용 가능한 상태가 된다. |
| Failed Post Condition | 시뮬레이션이 완료되지 않아 결과를 산출할 수 없다. |
| **MAIN SUCCESS SCENARIO** | |
| Step | Action |
...
| Concurrency | None |
| Etc | 도달 확률 = (해당 셀이 점화된 회차 수) / (전체 회차 수) |
...
| --- | --- |
| **GENERAL CHARACTERISTICS** | |
| Summary | 사용자가 시뮬레이션 결과를 층별 2D 확률 히트맵 형태로 확인하는 기능 |
| Scope | FireWatch |
| Level | User Level |
| Status | Analysis |
...
| Concurrency | None |
| Etc | 셀별 도달 확률은 0%(회색) → 50%(주황) → 100%(빨강) 그라데이션으로 표현됨 |
...
| --- | --- |
| **GENERAL CHARACTERISTICS** | |
| Summary | 사용자가 결과 화면에서 다른 층의 화재 확률 분포를 확인하기 위해 층을 전환하는 기능 |
| Scope | FireWatch |
| Level | User Level |
| Status | Analysis |
| Primary Actor | Commander, Field Operator |
...
| Success Post Condition | 선택한 층의 확률 분포 히트맵이 화면에 표시된다. |
| Failed Post Condition | 층 전환이 적용되지 않아 기존 화면이 유지된다. |
| **MAIN SUCCESS SCENARIO** | |
| Step | Action |
| 1 | 사용자가 층 선택 탭에서 보고 싶은 층을 선택한다. |
| 2 | 시스템이 선택된 층의 확률 분포 데이터를 가져와 화면을 갱신한다. |
| 3 | 사용자가 새 층의 결과를 확인한다. |
| **EXTENSION SCENARIOS** | |
| Step | Branching Action |
| | (UI 레벨에서 결과가 없을 때 층 전환 자체가 비활성화되므로 EXTENSION 분기가 없다.) |
| **RELATED INFORMATION** | |
| Performance | ≤ 1 Second (층 데이터 로드 + 화면 갱신) |
...
| --- | --- |
| **GENERAL CHARACTERISTICS** | |
| Summary | 사용자가 시간 슬라이더 등으로 특정 시점을 선택하여 해당 시점의 화재 확률 분포를 확인하는 기능 |
| Scope | FireWatch |
| Level | User Level |
| Status | Analysis |
| Primary Actor | Commander, Field Operator |
...
| Success Post Condition | 해당 시점의 확률 분포가 히트맵에 반영되어 표시된다. |
| Failed Post Condition | 시간 변경이 적용되지 않아 기존 시점이 유지된다. |
| **MAIN SUCCESS SCENARIO** | |
| Step | Action |
| 1 | 사용자가 시간 슬라이더를 조작하여 보고 싶은 시점을 선택한다. |
| 2 | 시스템이 선택된 시점의 셀별 확률 분포를 가져와 화면을 갱신한다. |
| 3 | 사용자가 해당 시점의 결과를 확인한다. |
| **EXTENSION SCENARIOS** | |
| Step | Branching Action |
| | (UI 레벨에서 결과가 없을 때 시간 슬라이더 자체가 비활성화되므로 EXTENSION 분기가 없다.) |
| **RELATED INFORMATION** | |
| Performance | ≤ 1 Second (시점 데이터 로드 + 화면 갱신) |
| Frequency | 사용자 조작 시마다 |
| Concurrency | None |
| Etc | |
...
| --- | --- |
| **GENERAL CHARACTERISTICS** | |
| Summary | 사용자가 건물의 층별 위험도를 단면도 형태로 한눈에 확인하는 기능 |
| Scope | FireWatch |
| Level | User Level |
| Status | Analysis |
| Primary Actor | Commander, Field Operator |
...
| Success Post Condition | 건물의 측면 단면도가 표시되며, 각 층이 위험도에 따라 색상으로 구분되어 보인다. |
| Failed Post Condition | 단면도가 표시되지 않거나 위험도가 잘못 표시된다. |
| **MAIN SUCCESS SCENARIO** | |
| Step | Action |
| 1 | 사용자가 단면도 보기 기능을 실행한다. |
| 2 | 시스템이 각 층별로 도달 확률 상위 10% 셀의 평균값을 계산한다. |
| 3 | 시스템이 단면도와 층별 위험도 색상을 화면에 표시한다. |
| **EXTENSION SCENARIOS** | |
| Step | Branching Action |
| | (UI 레벨에서 결과가 없을 때 단면도 보기 자체가 비활성화되므로 EXTENSION 분기가 없다.) |
| **RELATED INFORMATION** | |
...
| Success Post Condition | 시뮬레이션 결과의 정량 수치가 표 또는 차트 형태로 화면에 표시된다. |
| Failed Post Condition | 수치 요약이 표시되지 않거나 잘못된 값이 표시된다. |
| **MAIN SUCCESS SCENARIO** | |
| Step | Action |
| 1 | 사용자가 결과 요약 패널을 연다. |
| 2 | 시스템이 EnsembleResult로부터 핵심 통계 수치를 계산한다. |
| 3 | 시스템이 계산된 수치들을 표 또는 차트 형태로 화면에 표시한다. |
| **EXTENSION SCENARIOS** | |
| Step | Branching Action |
| | (UI 레벨에서 결과가 없을 때 요약 패널 자체가 비활성화되므로 EXTENSION 분기가 없다.) |
| **RELATED INFORMATION** | |
...
| --- | --- |
| **GENERAL CHARACTERISTICS** | |
| Summary | 관제관이 다른 파라미터 조합으로 시뮬레이션을 추가 실행하고, 두 시나리오의 결과를 시각적으로 비교하는 기능 |
| Scope | FireWatch |
| Level | User Level |
| Status | Analysis |
| Primary Actor | Commander |
| Secondary Actors | None |
| Preconditions | Run Ensemble(Use Case ID: #4) 결과가 적어도 하나 이상 존재해야 한다. |
| Trigger | 관제관이 시나리오 비교 기능을 실행할 때 |
| Success Post Condition | 두 시나리오의 결과가 나란히 표시되어 비교 가능하다. |
| Failed Post Condition | 비교가 수행되지 않아 단일 결과만 표시된다. |
| **MAIN SUCCESS SCENARIO** | |
| Step | Action |
...
| --- | --- |
| **GENERAL CHARACTERISTICS** | |
| Summary | 관제관이 현재의 시뮬레이션을 종료하는 기능 |
| Scope | FireWatch |
| Level | User Level |
| Status | Analysis |
| Primary Actor | Commander |
...
| Success Post Condition | 현재 시뮬레이션의 모든 결과와 입력값이 제거되고 메인 화면으로 복귀한다. |
| Failed Post Condition | 초기화가 정상적으로 수행되지 않아 시스템 상태가 일관되지 않게 된다. |
| **MAIN SUCCESS SCENARIO** | |
| Step | Action |
| 1 | 관제관이 초기화 버튼을 누른다. |
| 2 | 시스템이 초기화 확인 팝업을 띄워 의사를 재확인한다. |
| 3 | 관제관이 확인을 누른다. |
| 4 | 시스템이 메모리상의 신고 정보, 파라미터, 시뮬레이션 결과를 모두 제거하고 메인 화면으로 복귀한다. |
| **EXTENSION SCENARIOS** | |
| Step | Branching Action |
| 3 | 3a. 관제관이 취소를 누른 경우.<br>　3a1. 초기화를 수행하지 않고 기존 화면으로 돌아간다.<br>3b. 시뮬레이션이 진행 중인 경우.<br>　3b1. 진행 중인 시뮬레이션을 강제 종료한다. |
| **RELATED INFORMATION** | |
| Performance | ≤ 2 Seconds (메모리 정리 및 화면 복귀까지의 시간) |
| Frequency | 사용자 요청 시 |
| Concurrency | None |
| Etc | |
...
시뮬레이션 결과의 시각화(히트맵 색상 그라데이션, 단면도 색상 등)와 파생 통계 계산(층별 상위 10% 평균, 화재 확산 면적, 구역별 평균 확률 등)은 EnsembleResult로부터 파생되는 작업이며, 구체적인 시각화 책임은 4. UI prototype에서 다룬다. 본 도메인 분석에서는 데이터 구조와 핵심 도메인 로직에 집중한다.
...
![FireWatch Main Screen](images/screenshot_01_main.png)
...
| **Use Case** | 사용자(액터)가 시스템과 상호작용하여 달성하는 하나의 기능적 단위. UML의 핵심 모델링 요소이다. |
| **«include»** | 한 Use Case가 다른 Use Case를 본체 로직 안에서 반드시 호출하는 관계. |
| **«extend»** | 한 Use Case가 다른 Use Case의 화면 또는 흐름 위에서 조건부로 분기되는 관계. |
| **Mock Data** | 실제 시스템 동작을 흉내내기 위한 가짜 데이터. 분석 단계의 prototype에서 동작 가능성 시연을 위해 사용된다. |
| **관제관 (Commander)** | FireWatch 시스템의 주 사용자. 신고 접수, 시뮬레이션 실행, 시나리오 비교 등 모든 제어 권한을 가진다. |
| **현장요원 (Field Operator)** | 시뮬레이션 결과를 현장에서 확인하는 사용자. 결과 조회와 자기 디바이스에서의 층 전환·시간 조작만 가능하다. |
...
2. Karafyllidis, I. & Thanailakis, A., "A model for predicting forest fire spreading using cellular automata", *Ecological Modelling*, 99(1), 87-97, 1997.
3. Trunfio, G. A., "Predicting Wildfire Spreading Through a Hexagonal Cellular Automata Model", *International Conference on Cellular Automata*, 385-394, 2004.
...
5. Fowler, M., *UML Distilled: A Brief Guide to the Standard Object Modeling Language* (3rd Edition), Addison-Wesley Professional, 2003.
6. Riverbank Computing, "PyQt6 Reference Guide", 2024 — https://www.riverbankcomputing.com/static/Docs/PyQt6/
7. StarUML, "Use Case Diagram Documentation" — https://docs.staruml.io/working-with-uml-diagrams/use-case-diagram
